...
def history_stock_daily(ts_code='', adjust="", trade_date=""):
    ...
    (%s, %s, %s, %s, %s, %s, %s, %s, %s,%s);
    """
    if stock_zh_a_daily_hfq_df is None or len(stock_zh_a_daily_hfq_df) == 0:
        print('ts_code', ts_code, 'adjust', adjust)
        return False
    for index, row in stock_zh_a_daily_hfq_df.iterrows():
        ...
        ak_conn.commit()
    print("success insert akshare_daily ", ts_code, adjust, trade_date, ' cost', time.time() - start_time)
    return False
...
def dowload_stock_daily(input_start_date):
    print("input start date", input_start_date)
    ...
    for ts_code in code_list:
        print("ready download ts_code:" + ts_code)
        for adjust in adjust_list:
            try:
                is_retry = history_stock_daily(ts_code, adjust, input_start_date)
                # time.sleep(1)
            except Exception as e:
                print(e)
...
if __name__ == '__main__':
    ...
    check_akshare_stock_daily('20200731')
